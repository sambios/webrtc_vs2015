...
#include "webrtc/modules/audio_coding/codecs/aac/audio_encoder_aac.h"
...
AudioEncoderAAC::Config CreateConfig(const CodecInst& codec_inst) {
  ...
  config.application = config.num_channels == 1 ? AudioEncoderAAC::kVoip
      : AudioEncoderAAC::kAudio;
  return config;
}
...
AudioEncoderAAC::AudioEncoderAAC(const CodecInst& codec_inst)
    : AudioEncoderAAC(CreateConfig(codec_inst)) {}

AudioEncoderAAC::~AudioEncoderAAC() {
  RTC_CHECK_EQ(0, WebRtcAAC_EncoderFree(inst_));
}
...
size_t AudioEncoderAAC::NumChannels() const {
  return config_.num_channels;
}

size_t AudioEncoderAAC::Num10MsFramesInNextPacket() const {
  return Num10msFramesPerPacket();
}

size_t AudioEncoderAAC::Max10MsFramesInAPacket() const {
  return Num10msFramesPerPacket();
}
...
void AudioEncoderAAC::Reset() {
  RTC_CHECK(RecreateEncoderInstance(config_));
}

bool AudioEncoderAAC::SetFec(bool enable) {
  auto conf = config_;
  conf.fec_enabled = enable;
  return RecreateEncoderInstance(conf);
}

bool AudioEncoderAAC::SetDtx(bool enable) {
  auto conf = config_;
  conf.dtx_enabled = enable;
  return RecreateEncoderInstance(conf);
}

bool AudioEncoderAAC::SetApplication(Application application) {
  auto conf = config_;
  switch (application) {
    case Application::kSpeech:
      conf.application = AudioEncoderAAC::kVoip;
      break;
    case Application::kAudio:
      conf.application = AudioEncoderAAC::kAudio;
      break;
  }
  return RecreateEncoderInstance(conf);
}

void AudioEncoderAAC::SetMaxPlaybackRate(int frequency_hz) {
  auto conf = config_;
...
void AudioEncoderAAC::SetTargetBitrate(int bits_per_second) {
  ...
  RTC_CHECK_EQ(0, WebRtcAAC_SetBitRate(inst_, config_.bitrate_bps));
}

AudioEncoder::EncodedInfo AudioEncoderAAC::EncodeImpl(
...
size_t AudioEncoderAAC::Num10msFramesPerPacket() const {
  return static_cast<size_t>(rtc::CheckedDivExact(config_.frame_size_ms, 10));
}

size_t AudioEncoderAAC::SamplesPer10msFrame() const {
  return rtc::CheckedDivExact(kSampleRateHz, 100) * config_.num_channels;
}

size_t AudioEncoderAAC::SufficientOutputBufferSize() const {
...
// false.
bool AudioEncoderAAC::RecreateEncoderInstance(const Config& config) {
    if (!config.IsOk())
        return false;
    if (inst_)
        RTC_CHECK_EQ(0, WebRtcAAC_EncoderFree(inst_));
    ...
    RTC_CHECK_EQ(0, WebRtcAAC_EncoderCreate(&inst_,AAC_AOT_LC,
        kSampleRateHz,
        config.num_channels));
    RTC_CHECK_EQ(0, WebRtcAAC_SetBitRate(inst_, config.bitrate_bps));
    if (config.fec_enabled) {
        RTC_CHECK_EQ(0, WebRtcAAC_EnableFec(inst_));
    }
    else {
        RTC_CHECK_EQ(0, WebRtcAAC_DisableFec(inst_));
    }
    //RTC_CHECK_EQ(
    //    0, WebRtcAAC_SetMaxPlaybackRate(inst_, config.max_playback_rate_hz));
    RTC_CHECK_EQ(0, WebRtcAAC_SetComplexity(inst_, config.complexity));
    if (config.dtx_enabled) {
        //RTC_CHECK_EQ(0, WebRtcAAC_EnableDtx(inst_));
    }
    else {
        //RTC_CHECK_EQ(0, WebRtcAAC_DisableDtx(inst_));
    }
    RTC_CHECK_EQ(0,
